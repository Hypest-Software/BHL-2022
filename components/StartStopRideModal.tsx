...
export default function StartStopRideModal(props: ModalProps) {
    ...
    const getBoughtTickets = () => {
        return boughtTickets.data.boughtTickets.filter((boughtTicket) => boughtTicket.ticketCount > 0).map((boughtTicket) => {
            return {
                ...boughtTicket,
                ...tickets.data.tickets.find((ticket) => ticket.id === boughtTicket.ticketId)
            }
        })
    }
                        ...
                        <div className="relative w-auto my-6 mx-auto max-w-3xl">
                            <div
                                ...
                                <div
                                    ...
                                    <h3 className="text-3xl font-semibold">Rozpocznij przejazd</h3>
                                    <button
                    ...
                    <span
                      ...
                      ×
                                    ...
                                    <div className="form-control w-full max-w-xs">
                                        <div className="dropdown">
                                            <label tabIndex={Number(0)} className="btn m-1">
                                                {ticket ? ticket.name + ' - ' + 'x' + ticket.ticketCount : (tickets.data && getBoughtTickets().length !== 0 ? 'Wybierz bilet' : 'Brak biletów')}
                                            </label>
                                            <ul
                                                tabIndex={Number(0)}
                                                className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52"
                                            >
                                                {tickets.data && getBoughtTickets().map((ticket) => (
                                                    <li key={ticket.id}>
                                                        <a
                                ...
                                <div
                                    ...
                                    <button
                                        disabled={boughtTickets.data && getBoughtTickets().length == 0}
